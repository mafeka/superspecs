...
**Input**: Optionally a change name via `$ARGUMENTS` (e.g.
`/superspec-tasks token-refresh`). If omitted, infer from conversation
context; if ambiguous, run `openspec list --json` and ask the user
directly to let them pick. Announce "Using change: <name>".
