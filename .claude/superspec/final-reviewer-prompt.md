...
Dispatch once via the Agent tool, `subagent_type: general-purpose`, fresh
context, no proposal.md — after every task in the change has an Approved
task review, before `/superspec-archive` is allowed to run. This gate is
adversarial by design: an independent context catches what per-task review
structurally cannot, because per-task review never sees the whole diff.
